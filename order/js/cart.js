...
let TAX_RATE = 0.1; // デフォルト10%

// === 税率を設定ファイルから取得 ===
function fetchTaxRate(){
    const url = '../admin/adminsetting_registrer.php?section=order_settings';
    return fetch(url).then(r=>r.json()).then(j=>{
        if(j && j.success && j.settings && typeof j.settings['tax rate']!=='undefined'){
            const ratePercent = parseFloat(j.settings['tax rate']);
            if(!isNaN(ratePercent)){
                TAX_RATE = ratePercent/100;
                console.log('[設定] TAX_RATE 更新',TAX_RATE);
            }
        }
    }).catch(err=>{
        console.warn('[税率取得失敗]',err);
    });
}
...
function setupCartEventListeners() {
    // 旧実装で想定されるカートタブボタンの処理
    const cartTabBtn = document.querySelector('.tab[data-tab="cart"]');
    if (cartTabBtn) {
        cartTabBtn.addEventListener('click', function() {
            console.log('カートタブをクリック (旧実装セレクタ)');
            if (typeof updateCartUI === 'function') {
                updateCartUI(); // カートUIを更新
            }
            // 必要であれば、ここでカートモーダル表示処理などを追加
        });
    } else {
        console.log('旧実装のカートタブボタン (.tab[data-tab="cart"]) は見つかりませんでした。');
    }

    // フッターの「注文へ進む」ボタンの処理 (現行index.phpに存在)
    const viewCartButton = document.getElementById('view-cart-button');
    if (viewCartButton) {
        viewCartButton.addEventListener('click', function() {
            console.log('フッターの「注文へ進む」ボタン (#view-cart-button) がクリックされました。');
            // カートモーダル (cartModal) を表示する想定
            // ui.js に showModal(document.getElementById('cartModal')) のような関数があるか、
            // または直接ここでモーダル表示ロジックを記述
            if (typeof showModal === 'function' && document.getElementById('cartModal')) {
                console.log('cartModal を表示します。');
                ...
                console.warn('showModal関数または #cartModal が見つかりません。カートモーダルを表示できません。');
                //代替としてカートUIのみ更新を試みる
                 if (typeof updateCartUI === 'function') {
                    updateCartUI(); 
                }
        ...
        console.log('フッターの「注文へ進む」ボタン (#view-cart-button) が見つかりませんでした。');
    }
...
function addToCart(product, quantity = 1) {
    // 文字列(ID) が渡ってきた場合は商品オブジェクトを取得
    if((typeof product==='string' || typeof product==='number') && typeof getProductByIdLocal==='function'){
        const tmp = getProductByIdLocal(product);
        if(tmp){
            product = tmp;
        }
    }
...
function updateCartUI() {
    ...
    // 商品カードのバッジも常に更新
    if (typeof updateProductBadges === 'function') {
        updateProductBadges();
    }
    ...
    // フッター合計数・金額を更新 (#total-quantity / #total-price)
    const totalQtyEl = document.getElementById('total-quantity');
    const totalPriceEl = document.getElementById('total-price');
    if (totalQtyEl) totalQtyEl.textContent = itemCount;

    const totalPrice = calculateTotal();
    if (totalPriceEl) totalPriceEl.innerHTML = `¥${totalPrice.toLocaleString()}-<span class="tax-note">(税抜)</span>`;
    
    // 注文へ進むボタン (#view-cart-button) の活性/非活性
    const viewCartBtn = document.getElementById('view-cart-button');
    if (viewCartBtn) viewCartBtn.disabled = itemCount === 0;
...
function updateCartModal() {
    const cartItemsContainer = document.getElementById('final-cart-items-container');
    if (!cartItemsContainer) return;
    
    // --- アイテムリスト描画 ---
    let html = '';
    if (cartItems.length === 0) {
        html = '<p class="empty-cart">カートに商品がありません</p>';
    } else {
        cartItems.forEach(item => {
            const el = createCartItemElement(item);
            html += el.outerHTML; // createCartItemElement で生成→HTML取得
        });
    }
    ...
    html += `
        <div class="cart-total-section">
            <div class="cart-total-row"><span>小計</span><span id="finalCartSubtotal">¥${subtotal.toLocaleString()}</span></div>
            <div class="cart-total-row"><span>消費税(${taxPercent}% )</span><span id="finalCartTax">¥${tax.toLocaleString()}</span></div>
            <div class="cart-total-row total"><span>合計</span><span id="finalCartTotal">¥${total.toLocaleString()}</span></div>
        </div>`;
    // メモ入力欄
    html += `
        <div class="notes-section">
            <label for="orderMemo">スタッフへの連絡</label>
            <textarea id="orderMemo" rows="5" placeholder="スタッフへの連絡"></textarea>
        </div>`;
...
function createCartItemElement(item) {
    ...
    itemElement.innerHTML = `
        <div class="cart-item-image">
            <img src="${item.image_url || item.image || 'images/no-image.png'}" alt="${item.name}">
        </div>
...
function createOrderData(notes = '') {
    ...
    const lineUserId = window.LINE_USER_ID || (typeof userProfile !== 'undefined' && userProfile ? userProfile.userId : null);
    
    // notes 引数にメモを受け取り、そのまま payload へ反映します
    const orderData = {
        ...
        items: cartItems.map(item => ({
            ...
            price: item.price,
            square_item_id: item.square_item_id || item.squareId || null
        })),
...
function populateOrderConfirmation() {
    const listEl = document.getElementById('order-confirmation-list-items');
    const totalEl = document.getElementById('confirmation-total-amount');
    ...
    cartItems.forEach(item => {
        ...
        li.innerHTML = `
            ...
            <div class="confirm-item-subtotal">¥${sub.toLocaleString()}</div>`;
...
async function placeOrder() {
    try {
        if (!window.apiClient || typeof window.apiClient.submitOrder !== 'function') {
            throw new Error('APIクライアントが初期化されていません');
        }

        /* === Square 設定チェック === */
        let openTicketFlag = null;
        try {
            if(typeof window.apiClient.getSquareSettings==='function'){
                const sqSet = await window.apiClient.getSquareSettings();
                if(sqSet && typeof sqSet.open_ticket!=='undefined'){
                    openTicketFlag = !!sqSet.open_ticket;
                    if(openTicketFlag){
                        console.log('%c squareオーダーモード=open_ticketモードが有効です、openTicketモードで処理します','color:green');
                    }else{
                        console.log('%c squareオーダーモード=open_ticketモードが無効です、productモードで処理します','color:orange');
                    }
                } else {
                    console.warn('squareオーダーモード=確認できません、再度読み取りを行います');
                }
            }
        } catch(setErr){
            console.error('squareオーダーモード=再チェックで確認できません、squareモード確認エラー',setErr);
        }

        const memoInput = document.getElementById('orderMemo');
        const memoText = memoInput ? memoInput.value.trim() : '';
        const orderData = createOrderData(memoText);
        console.log('送信する注文データ:', orderData);
        const response = await window.apiClient.submitOrder(orderData);
        console.log('注文APIレスポンス', response);
        if (response && response.success) {
            clearCart();
            // メモ入力欄をリセット
            if(document.getElementById('orderMemo')){
                document.getElementById('orderMemo').value='';
            }
            return { success:true, orderNumber: response.order_id || response.orderNumber || '', sessionId: response.session_id || null };
        } else {
            return { success:false, message: response.message || '注文に失敗しました' };
        }
    } catch (e) {
        console.error('placeOrder error', e);
        return { success:false, message: e.message || '通信エラー' };
    }
}

window.placeOrder = placeOrder;
window.populateOrderConfirmation = populateOrderConfirmation;

// カスタムイベント発行
function dispatchCartUpdatedEvent(){
    try{
        const evt = new CustomEvent('cart-updated',{detail:{items:[...cartItems]}});
        document.dispatchEvent(evt);
    }catch(e){
        console.warn('dispatchCartUpdatedEvent error',e);
    }
} 