<?php
/**
 * OpenTicketモード用注文サービスクラス
 * バージョン: 1.0.0
 * ファイル説明: OpenTicketモードでの注文処理を専門に扱うクラス
 */
...
class OrderService_Openticket implements OrderServiceInterface {
    private $db;
    ...
    public function __construct() {
        // ログファイルの初期化
        self::initLogFile();
        self::logMessage("OrderService_Openticket::__construct - OpenTicketモード初期化開始", 'INFO');
        ...
        self::logMessage("OrderService_Openticket::__construct - OpenTicketモード初期化完了", 'INFO');
    }
    ...
    private static function initLogFile() {
        ...
        self::$logFile = $logDir . '/OrderService_Openticket.log';
    ...
    public function createOrder($roomNumber, $items, $guestName = '', $note = '', $lineUserId = '') {
        ...
        self::logMessage("createOrder[OpenTicket] 開始 - 引数: " . self::formatArgs($argsLog), 'INFO');
        ...
        try {
            ...
            // RoomTicketServiceを使用して保留伝票に商品を追加
            self::logMessage("RoomTicketServiceを使用して保留伝票に商品を追加します: " . count($orderData['items']) . "件", 'INFO');
            
            $updatedTicket = $this->roomTicketService->addItemToRoomTicket($roomNumber, $orderData['items']);
            ...
            self::logMessage("保留伝票更新成功: TicketID=" . ($updatedTicket['square_order_id'] ?? 'N/A'), 'INFO');
            ...
            $orderId = $this->saveOrderToDatabase(
                ...
                $subtotalAmount,
                $note,
                $updatedTicket['square_order_id'] ?? null,
                $orderData['items']
            );
            ...
            $result = [
                'id' => $orderId,
                'square_order_id' => $updatedTicket['square_order_id'] ?? null,
                'room_number' => $roomNumber,
                'total_amount' => $subtotalAmount,
                'status' => 'OPEN',
                'created_at' => date('Y-m-d H:i:s'),
                'order_session_id' => null, // OpenTicketモードではセッションIDは使用しない
                'square_item_id' => null,    // OpenTicketモードではSquareItemIDは使用しない
                'note' => $note
            ];
            ...
            self::logMessage("createOrder[OpenTicket] 完了 - OrderID: {$orderId}", 'INFO');
            return $result;
            
        } catch (Exception $e) {
            self::logMessage("createOrder[OpenTicket] エラー: " . $e->getMessage(), 'ERROR');
            return false;
        }
    }
     ...
     * 注文をデータベースに保存
     */
    private function saveOrderToDatabase($roomNumber, $guestName, $lineUserId, $subtotalAmount, $note, $squareOrderId, $orderItems) {
        ...
        try {
            ...
            foreach ($orderItems as $item) {
                $this->db->execute(
                    ...
                    [
                        $orderId,
                        null, // OpenTicketモードではセッションIDは使用しない
