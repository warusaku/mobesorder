...
API.prototype.submitOrder = function(orderData) {
    ...
    if(orderData && orderData.line_user_id){
        headers['X-LINE-USER-ID'] = orderData.line_user_id;
    }
...
// （★追加）カテゴリの営業時間をチェック
API.prototype.checkCategoryOpenStatus = function(categoryId) {
    if (!categoryId) {
        return Promise.reject(new Error('カテゴリIDが指定されていません'));
    }

    // openclose_manager.php は order ディレクトリ配下に配置されているため
    // フロントページ（/fgsquare/order/）からの相対パスで指定
    var url = 'api/lib/openclose_manager.php';
    var timestamp = new Date().getTime();
    url += '?category_id=' + encodeURIComponent(categoryId) + '&_=' + timestamp;
    ...
    return this.sendRequest(url)
        .then(function(data) {
            ...
            throw new Error('営業状態データが不正です');
        });
};

// （★追加）次回の営業開始時間を取得
API.prototype.getNextOpeningTime = function() {
    var url = 'api/lib/openclose_manager.php?action=next_open';
    var timestamp = new Date().getTime();
    url += '&_=' + timestamp;

    return this.sendRequest(url)
        .then(function(data) {
            if (data && data.next_open_time) {
                return data.next_open_time;
            }
            return null;
        });
};

// プロトタイプメソッド：Square設定を取得 (open_ticket フラグ等)
API.prototype.getSquareSettings = function(){
    // 管理者設定エンドポイント (ドキュメントルート直下)
    var url = '/admin/adminsetting_registrer.php?section=square_settings&_=' + (new Date().getTime());
    console.log('Square設定取得URL:',url);
    return this.sendRequest(url).then(function(data){
        if(data && data.settings){
            return data.settings; // {open_ticket: true/false, ...}
        }
