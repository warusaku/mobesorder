<?php
// AuthService を先に読み込む（存在する場合）
$authServicePath = __DIR__ . '/AuthService.php';
if (!class_exists('AuthService') && file_exists($authServicePath)) {
...
class Auth {
    ...
    public function __construct() {
        ...
        // AuthService クラスが存在しない場合は無視（後方互換）
        if (class_exists('AuthService')) {
            $this->authService = AuthService::getInstance();
        } else {
            $this->authService = null;
            if (function_exists('error_log')) {
                error_log('AuthService クラスが見つかりません。後方互換モードで継続します');
            }
        }
    }
    ...
    public function getRoomByLineUserId($lineUserId) {
        ...
        return $this->db->selectOne(
            "SELECT l.id, l.room_number, l.line_user_id, r.guest_name, r.check_in_date, r.check_out_date
             FROM line_room_links l
             LEFT JOIN room_tokens r ON l.room_number = r.room_number AND r.is_active = 1
             WHERE l.line_user_id = ? AND l.is_active = 1",
            [$lineUserId]
    ...
    public function getLineUserByRoomNumber($roomNumber) {
        ...
        // フォールバック: 直接DB検索
        return $this->db->selectOne(
            "SELECT l.id, l.room_number, l.line_user_id FROM line_room_links l WHERE l.room_number = ? AND l.is_active = 1 LIMIT 1",
            [$roomNumber]
        );
    }
} 