...
class ProductService {
    ...
    public function getProducts($activeOnly = true) {
        ...
        if ($activeOnly) {
            $query .= " WHERE is_active = 1 AND presence = 1 AND order_dsp = 1";
        } else {
            $query .= " WHERE presence = 1 AND order_dsp = 1";
        }
    ...
    public function getProductBySquareId($squareItemId) {
        ...
        // 価格フォーマットの補正（JPY は整数円で扱う）
        if (isset($product['price'])) {
            $originalPrice = $product['price'];
            // 日本円は小数を持たない通貨。小数点以下が存在する場合は四捨五入して整数化し、×100 補正は行わない。
            $product['price'] = (int)round($product['price']);
            if ($product['price'] != $originalPrice) {
                Utils::log("商品価格を補正: {$originalPrice} → {$product['price']}", 'DEBUG', 'ProductService');
            }
        }
    ...
    public function getProductsByCategoryId($categoryId, $activeOnly = true) {
        try {
            ...
            // カテゴリIDに属する商品を取得
            $query = "SELECT * FROM products WHERE category = ? AND presence = 1 AND order_dsp = 1 ";
            $params = [$categoryId];
    ...
    /**
     * 汎用 getProduct (後方互換)
     * OrderService 等の旧実装互換のため getProductById をエイリアス
     *
