...
<div class="container">
    ...
    <form method="post" enctype="multipart/form-data">
        <div class="form-group">
            <label>テンプレート名：</label>
            <input type="text" name="template_title" required>
        </div>
        <div class="form-group">
            <label>メッセージタイプ：</label>
            <div class="segmented-control">
                <input type="radio" name="message_type" id="type_text" value="text" checked>
                <label for="type_text">テキスト</label>
                <input type="radio" name="message_type" id="type_rich" value="rich">
                <label for="type_rich">リッチ</label>
            </div>
        </div>
        <div id="text_message_area" class="form-group">
            <label>送信メッセージ：</label>
            <textarea name="message" required></textarea>
        </div>
        <div id="rich_message_area" class="form-group" style="display:none;">
            <label>テンプレート選択：</label>
            <div id="template_select" style="display: flex; gap: 1em; margin-bottom: 1em;">
                <label style="text-align: center; cursor: pointer; flex:1;">
                    <input type="radio" name="template_type" value="block1" checked style="display:none;">
                    <img src="/lumos/template/block1-100.jpg" alt="1分割" style="width: 80px; border: 2px solid #2ecc40; border-radius: 4px; display: block; margin-bottom: 0.5em; max-width:100%;">
                    <div>通常</div>
                </label>
                <label style="text-align: center; cursor: pointer; flex:1;">
                    <input type="radio" name="template_type" value="block2yoko" style="display:none;">
                    <img src="/lumos/template/block2yoko-100.jpg" alt="横2分割" style="width: 80px; border: 2px solid #ccc; border-radius: 4px; display: block; margin-bottom: 0.5em; max-width:100%;">
                    <div>横2分割</div>
                </label>
                <label style="text-align: center; cursor: pointer; flex:1;">
                    <input type="radio" name="template_type" value="block2tate" style="display:none;">
                    <img src="/lumos/template/block2tate-100.jpg" alt="縦2分割" style="width: 80px; border: 2px solid #ccc; border-radius: 4px; display: block; margin-bottom: 0.5em; max-width:100%;">
                    <div>縦2分割</div>
                </label>
                <label style="text-align: center; cursor: pointer; flex:1;">
                    <input type="radio" name="template_type" value="block4" style="display:none;">
                    <img src="/lumos/template/block4-100.jpg" alt="4分割" style="width: 80px; border: 2px solid #ccc; border-radius: 4px; display: block; margin-bottom: 0.5em; max-width:100%;">
                    <div>4分割</div>
                </label>
            </div>
            <div id="template_links_area"></div>
            <label>タイトル：</label>
            <input type="text" name="rich_title">
            <span style="color:#e57373; font-size:0.95em;">※この内容はLINEの通知で表示されます</span>
            <label>画像：</label>
            <div id="drop_zone" style="border: 2px dashed #ccc; padding: 20px; text-align: center; margin-bottom: 1em; background: #fafafa; cursor: pointer;">
                <input type="file" name="rich_image" id="rich_image" accept="image/*" style="display: none;" onchange="previewImage(this);">
                <div id="drop_text">
            ...
            <div id="image_preview" style="margin: 1em 0; display: none;">
                ...
                <div>
                    <p>通常サイズ (1040×1040)</p>
                    <img id="preview_normal" style="max-width: 100%; border: 1px solid #ccc;">
                </div>
                <div id="image_info" style="margin-top: 1em; font-size: 0.9em; color: #666;"></div>
            </div>
            <span style="color:#e57373; font-size:0.95em;">※LINE公式リッチメッセージの基本サイズは <b>1040×1040px</b>（正方形）です。アップロードされた画像は自動的に正方形にリサイズされ、中央部分が切り取られます。ファイルサイズが1MBを超える場合は画質が自動的に調整されます。</span>
        </div>
        <button type="submit" style="width:100%;">テンプレートを保存</button>
    </form>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.querySelectorAll('input[name="message_type"]').forEach(function(el) {
            el.addEventListener('change', toggleMessageForm);
        });
        toggleMessageForm();
